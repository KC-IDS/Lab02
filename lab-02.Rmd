...
author: "the RegRession Rebels: Kenny Chen, Jake Leung, Matthew Brown and Aleksandra Iwaszkiewicz"
date: "`r Sys.Date()`"
...
```
Africa with 43 countries/territories
...
plastic_data_all %>% group_by(region) %>% summarise(avg = mean(total_mismanage_plastic))



```
See table for mean mismanaged plastic waste per region.
The lowest is Oceania and the highest is Asia(L contininent)

### Ex 3.

```{r Ex3, message = FALSE}
# Write your code here to answer exercise 3
plastic_data_all <- plastic_data_all %>% mutate(ct_mismanaged_plastic_ocean = mismanaged_plastic_ocean/total_mismanage_plastic*100)
plastic_data_all %>% group_by(region) %>% summarise(avg = median(ct_mismanaged_plastic_ocean))
